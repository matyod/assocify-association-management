<?php

require view_path('partials/head.php');
// require view_path('partials/nav.php');

?>

<div class="min-h-screen">
  <section class="min-h-screen bg-white dark:bg-gray-900 flex items-center">
    <div class="h-full py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
      <div class="mx-auto max-w-screen-sm text-center">
        <h1 class="mb-4 text-7xl tracking-tight font-extrabold lg:text-9xl text-sky-600 dark:text-sky-500">
          <?= $status ?? ''; ?>
        </h1>
        <p class="mb-4 text-3xl tracking-tight font-bold text-gray-900 md:text-4xl dark:text-white"><?= $title ?? ''; ?>
        </p>
        <p class="mb-4 text-lg font-light text-gray-500 dark:text-gray-400"><?= $message ?? ''; ?></p>
        <a href="/"
          class="inline-flex text-white bg-sky-600 hover:bg-sky-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:focus:ring-sky-900 my-4">Back
          to Homepage</a>
      </div>
    </div>
  </section>
</div>

<?php

// require view_path('partials/footer.php');
require view_path('partials/foot.php');

?>